...
class Mier:
    ...
    def get_request_from_keyword(self, headers, search_url, keyword, *args, **kwargs):
        # try:
            meta = {"url": search_url, "name": "douban", "platform": "douban", "keyword": keyword, "callback": "page_parse"}
            for i in range(1, 3):
                return scrapy.Request(url=search_url.format(i), headers=headers, meta=meta, dont_filter=False)
